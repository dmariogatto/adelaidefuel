...
namespace Xamarin.Forms.Maps.Android
{
    ...
    public class MapRenderer : ViewRenderer<Map, MapView>, IOnMapReadyCallback
    {
        ...
        protected virtual MarkerOptions CreateMarker(Pin pin)
        {
            ...
            var bitmap = GetMarkerBitmap(pin.FileImage, pin.TintColor);
        ...
        private void SetUserVisible()
        {
            ...
            if (MapModel.ShowUserLocation)
            {
                ...
                {
                    MapNative.MyLocationEnabled = MapNative.UiSettings.MyLocationButtonEnabled = true;
                }
                else
                {
                    Log.Warning("Xamarin.Forms.MapRenderer", "Missing location permissions for ShowUserLocation");
                    MapNative.MyLocationEnabled = MapNative.UiSettings.MyLocationButtonEnabled = false;
                }
            }
            else
            {
        ...
        private Bitmap GetMarkerBitmap(string fileImage, Color tint)
        {
            ...
            if (!string.IsNullOrEmpty(fileImage))
            {
                ...
                {
                    var cacheKey = $"TintedBitmap_{HashCode.Combine(fileImage, tint)}";
                    var filteredBitmap = default(Bitmap);

                    if (FormsMaps.Cache == null || !FormsMaps.Cache.TryGetValue(cacheKey, out filteredBitmap))
                    {
                        bitmap = File.Exists(fileImage)
                            ? BitmapFactory.DecodeFile(fileImage)
                            : Context.Resources.GetBitmap(fileImage, Context);
                        ...
                        FormsMaps.Cache?.SetSliding(cacheKey, filteredBitmap, TimeSpan.FromMinutes(2));
                    }
        ...
        private static void LoadMapStyle(GoogleMap map, MapTheme mapTheme, Context context)
        {
            ...
            var styleFileName = $"map.style.{mapTheme.ToString().ToLowerInvariant()}.json";

            //var resourceName = $"Xamarin.Forms.Maps.Platforms.android.Styles.map.style.{theme}.json";
            var assembly = typeof(MapRenderer).Assembly;
            var resourceName = assembly.GetManifestResourceNames().FirstOrDefault(n => n.EndsWith(styleFileName));

            if (!string.IsNullOrEmpty(resourceName) && !MapStyles.ContainsKey(resourceName))
            {
                using var stream = assembly.GetManifestResourceStream(resourceName);
                using var reader = new StreamReader(stream);
                MapStyles.AddOrUpdate(resourceName, new MapStyleOptions(reader.ReadToEnd()), (k, v) => v);
            }

            map.SetMapStyle(MapStyles[resourceName]);
        }
    }
}
